...
public class DeckOfCardQueue {
		...
		public static void main(String[] args) {
			...
			shuffleCard.shuffleCardsDeck(cardsArray);
			System.out.println("**************************************************************************************************************************");
			...
			{	
				String val=cardsArray[i];
				list.insertAtStart(queue.enqueue(val));
			...
			for(int i=0;i<cardsArray.length;i++)
			{
				...
				list.insertAtStart(queue.dequeue(val));
			}
			list.Sort(cardsArray);
			list.displayFromStart();
